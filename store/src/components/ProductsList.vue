    ...
    <template v-slot:default="{ items }">
      <v-row>
        <v-col v-for="(item, i) in items" :key="i" cols="12" sm="6" xl="3">
          <v-sheet border>
            <v-img :gradient="`to top right, rgba(255, 255, 255, .1), rgba(255, 255, 255, .15)`" :src="item.raw.img" cover
              height="150"></v-img>
            ...
            <v-table density="compact" class="text-caption">
              <tbody>
                ...
                <tr align="right">
                  <th></th>
                  <td>
                    <v-btn @click="$emit('productBtnClick', item.raw.productCode)">
                      {{ productBtnText }}
                    </v-btn>
                  </td>
                </tr>
...
<script>
export default {
  props: ["title", "products", "productBtnText"]
}
</script>
